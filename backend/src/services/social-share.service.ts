...
import { PrismaClient, SocialPlatform, ShareContentType, SocialShare } from '@prisma/client';
import { shareConfigs, ShareConfig } from '../config/social.config';
...
export class SocialShareService {
  ...
  async generateShareUrl(
    ...
    contentType: ShareContentType,
    contentId: string,
    customContent?: Partial<ShareContent>
  ...
  async generateAllShareUrls(
    ...
    const results = await Promise.all(
      platforms.map(platform =>
        this.generateShareUrl(userId, platform, contentType, contentId, customContent)
      )
    );
  ...
  private buildShareUrl(platform: SocialPlatform, content: ShareContent): string {
    const config = shareConfigs[platform.toLowerCase()];
    if (!config) {
      return content.url;
    }
    ...
    switch (platform) {
      case 'FACEBOOK':
        ...
        if (content.description) params.set('quote', content.description);
        break;
      ...
      case 'PINTEREST':
        ...
        if (content.imageUrl) params.set('media', content.imageUrl);
        break;
  ...
  // Track share click
  async trackClick(trackingId: string, referrer?: string): Promise<string | null> {
    ...
    const metadata = share.metadata as { originalUrl?: string };
    return metadata.originalUrl || null;
  }

  // Track conversion (signup, job created, etc.)
  async trackConversion(trackingId: string): Promise<void> {
  ...
  async getUserShareAnalytics(userId: string): Promise<ShareAnalytics> {
    ...
    const contentGroups = shares.reduce((acc, share) => {
      ...
      return acc;
    }, {} as Record<string, { contentType: ShareContentType; contentId: string; shares: number; clicks: number }>);
  ...
  async getUserShareHistory(
    userId: string,
    options: { limit?: number; offset?: number; platform?: SocialPlatform }
  ) {
  ...
  async getOpenGraphTags(contentType: ShareContentType, contentId: string): Promise<Record<string, string>> {
